...
### Добавлено
- Настройка пути к файлу статуса OpenVPN через конфигурацию    
    - Добавлена переменная окружения `OPENVPN_STATUS_FILE_NAME` в `.env_example`    

### Изменено
- Динамическое определение имени сервиса OpenVPN при перезапуске
    - Заменено захардкоженное значение `"openvpn@server"` на динамическое использование имени файла конфигурации без расширения    
    - Теперь сервис перезапускается с использованием имени из `OPENVPN_CONFIG_FILE_NAME` (например, `openvpn@vpn` для `vpn.conf`)
